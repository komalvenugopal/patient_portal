...
const AppointmentTable = () => {
    ...
    // Safeguard for contextData.date
    const date =
        contextData?.date &&
        `${contextData.date.getDate()}-${contextData.date.getMonth() + 1}-${contextData.date.getFullYear()}`;

    const makeBooking = (patientInfo) => {
        setIsBooked(true);
        if (!selectAppointment) return; // Ensure selectAppointment is defined
        const apId = uuidv4();
    ...
    const modalController = (apId) => {
        ...
        if (selectedAp) {
            ...
            console.warn(`Appointment with ID ${apId} not found.`);
        }
    };
        ...
        <div className="appointments container py-5 mt-5">
            ...
            <div className="row">
                ...
                {contextAppointmentData?.allAppointments?.map((SingleAp) => (
                    <AppointmentCard key={SingleAp.id} data={SingleAp} modalController={modalController} />
                ))}
            </div>
                ...
                {isBooked ? (
                    <div className="text-center py-5 my-5">
                        ...
                        <p className="mt-5 px-3">
                            Please go to Dashboard and Login with your email which you provided during booking to view details.
                        </p>
