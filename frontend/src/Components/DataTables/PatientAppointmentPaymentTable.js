...
const PatientAppointmentPaymentTable = () => {
    ...
    const handlePaymentSuccess = (paymentID) => {
        ...
        SelectedApForModify.status = 'Confirmed';
        ...
        const paymentData = { apId: SelectedApForModify.apId, paymentID, status: 'Confirmed' };
                ...
                <tbody>
                    {appointmentsOfThePatient.map((ap) => (
                        <tr key={ap.apId}>
                            ...
                            <td>{ap.date}</td>
                            <td>{ap.time}</td>
                            <td>{`#${ap.apId.substr(0, 7)}`}</td>
                            <td>
                                <button
                                    onClick={() => openDataDoctorModal(ap.apId, ap.apId)}
                            ...
                            <td className="text-center">
                                {ap.paymentID ? (
                                    <button onClick={() => openPaymentModal(ap.apId)} className="btn btn-primary">
                                        Paid
                                    </button>
                                ) : (
                                    <span>
                                        <span>Not Pay</span>
                                        <FontAwesomeIcon
                                            onClick={() => openPaymentModal(ap.apId)}
